...
class TaggingUtil {
	...
	public static function decrementCount($tagString, $tagSlug, $count) {
		...
		if($tag) {
			...
			if($tag->count < 0) {
				$tag->count = 0;
				\Log::warning("The \Xtwoend\Component\Category\Entities\Tag count for `$tag->name` was a negative number. This probably means your data got corrupted. Please assess your code and report an issue if you find one.");
			}
			$tag->save();
		}
	}
